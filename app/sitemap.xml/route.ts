...
/*
 * app/sitemap.xml/route.ts — Route Handler generating standard XML sitemap
 * with <?xml-stylesheet?> processing instruction so browsers render a clean,
 * formatted table via /sitemap.xsl while search engines parse standard XML.
 */

const PAGES = [
  { path: "", priority: 1.0, fallback: 0.8 },
...
// Stable build/release date for static pages
const STATIC_LAST_MOD = new Date(
  process.env.NEXT_PUBLIC_BUILD_TIME ?? "2026-08-16T00:00:00.000Z",
);
...
export async function GET() {
  const staticEntries = PAGES.flatMap((page) =>
    entriesFor(page.path, STATIC_LAST_MOD, page.priority, page.fallback),
  );
